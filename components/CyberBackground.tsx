...
const CyberBackground: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      // Theme Detection (Simple check for dark mode class)
      const isDark = document.documentElement.classList.contains('dark');
      const accentColor = isDark ? '6, 182, 212' : '59, 130, 246'; // Cyan vs Blue
      const gridOpacity = isDark ? 0.05 : 0.03;

      // 1. Draw Moving Grid (Vertical Scan)
      ctx.strokeStyle = `rgba(${accentColor}, ${gridOpacity})`;
      ctx.lineWidth = 1;
      const gridSize = 100;
      ...
      ctx.strokeStyle = `rgba(${accentColor}, 0.03)`;
      ctx.lineWidth = 2;
      ...
      ctx.fillStyle = `rgba(${accentColor}, 0.02)`;
      ...
      particles.forEach((p, i) => {
        ...
        if (distMouse < 300) {
           ctx.beginPath();
           ctx.strokeStyle = `rgba(${accentColor}, ${0.4 * (1 - distMouse / 300)})`;
           ctx.lineWidth = 1;
           ctx.moveTo(p.x, p.y);
        ...
        if (p.pulse > 0) {
            ...
            ctx.strokeStyle = `rgba(${accentColor}, ${Math.max(0, 0.8 - p.pulse / 50)})`;
            ctx.stroke();
            p.pulse += PULSE_SPEED;
        ...
        for (let j = i + 1; j < particles.length; j++) {
          ...
          if (dist < CONNECT_DISTANCE) {
            ctx.beginPath();
            ctx.strokeStyle = `rgba(${accentColor}, ${0.15 * (1 - dist / CONNECT_DISTANCE)})`;
            ctx.lineWidth = 0.5;
            ctx.moveTo(p.x, p.y);
            ...
            if ((i + j) % 8 === 0) {
                ...
                ctx.fill();
                ctx.fillStyle = `rgb(${accentColor})`; // Reset
            }
          }
        }
      });
  ...
  return (
    <canvas 
      ref={canvasRef} 
      className="fixed inset-0 z-[-1] bg-slate-50 dark:bg-[#020617] transition-colors duration-500 pointer-events-none"
    />
  );
};

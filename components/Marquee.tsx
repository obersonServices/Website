...
const Marquee: React.FC<MarqueeProps> = ({ items, direction = 'left', speed = 20 }) => {
  return (
    <div className="relative flex overflow-hidden py-4 bg-slate-100/50 dark:bg-slate-900/50 border-y border-slate-200 dark:border-slate-800">
      ...
      >
        {[...items, ...items, ...items, ...items].map((item, index) => (
          <div key={index} className="mx-8 flex items-center">
            <span className="text-xl md:text-2xl font-display font-bold text-slate-300 dark:text-slate-700 uppercase tracking-widest">
              {item}
            </span>

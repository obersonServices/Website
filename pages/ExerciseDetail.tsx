...
import { ArrowLeft, Clock, Users, FileText, Target, ClipboardList, TrendingUp } from 'lucide-react';
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          ...
          <div className="lg:col-span-2 space-y-12">
            ...
            <section>
              ...
              <p className="text-lg text-slate-600 dark:text-slate-300 leading-relaxed bg-white dark:bg-slate-900 p-8 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
              ...
              <div className="relative border-l-2 border-slate-300 dark:border-slate-700 ml-4 space-y-8">
                 {exercise.methodology.map((step, idx) => (
                   <div key={idx} className="relative pl-8">
                     <span className="absolute -left-[9px] top-1 h-4 w-4 rounded-full bg-brand-light dark:bg-brand-dark border-2 border-brand-accent"></span>
            ...
            {/* Objectives */}
            <section className="bg-slate-50 dark:bg-slate-900/50 p-8 rounded-xl border border-slate-200 dark:border-slate-800">
                <h3 className="text-xl font-bold text-slate-900 dark:text-white flex items-center mb-6">
                  <Target className="mr-3 text-brand-success" /> Key Objectives
                </h3>
             ...
             <div className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
                ...
                <ul className="space-y-3">
                  {exercise.participants.map((person, i) => (
                    <li key={i} className="flex items-center text-slate-600 dark:text-slate-300 text-sm bg-slate-100 dark:bg-slate-800 px-3 py-2 rounded">
                      {person}
                    </li>
                  ))}
                </ul>
